...
namespace QuanLyBanAn
{
    public partial class BanHang : Form
    {
       ...
       void  CTHD()
        ...
        {
            if (Convert.ToInt32( HienThiThanhTien.Text )> 0)
            {
                MessageBox.Show("Tổng Tiền của Đại Lý " + TenDaiLy.Text + " Đã Thanh Toán  " + HienThiThanhTien.Text);
                update();
                LoadHoaDon();
                sum();
            }
            else {
                MessageBox.Show("Bạn Không Có đơn Hàng Nào ");
                    }
        }
        ...
        {
            Con.Open();
            SqlCommand cmd = new SqlCommand("Select MaNhanVien from NhanVien Where TaiKhoan='"+Login.User+"'", Con);
            SqlDataReader Rdr;
